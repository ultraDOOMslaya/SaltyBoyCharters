export const paleGreen = '#61A644';
export const sproutGreen = '#AED29E';
export const appleGreen = '#579B3B';
export const successGreen = '#45A545';
export const paleSuccessGreen = '#ACD3AC';

export const blueStone = '#164C60';
export const lightBlue = '#005F9B';
export const darkBlue = '#163962';
export const slateBlue = '#122E41';
export const paleBlue = '#91B4CC';

export const lightGray = '#A2AAAD';
export const mediumGray = '#707070';
export const darkGray = '#525759';
export const slateGray = '#323E48';
export const silver = '#C5C5C5';
export const gainsboro = '#DCDCDC';
export const whiteSmoke = '#ECECEC';

export const hazyBlack = '#1E1E1E';

export const pureWhite = '#FFFFFF';
export const pureBlack = '#000000';

export const red = '#FF0000';
export const darkRed = '#AB0000';
export const lightRed = '#FF9999';